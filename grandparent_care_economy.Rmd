...
# Load demographic data to identify household structure
demo_data <- read_dta("TimeUse106DEMO.dta",
                      col_select = c(FSU_Serial_No, Sample_HH_No, Person_Serial_No,
                                    Relation_to_Head, Age, Gender, Marital_Status,
                                    Highest_Education, Principal_Activity_Status))

# Create household-level grandparent indicators
household_structure <- demo_data %>%
  group_by(FSU_Serial_No, Sample_HH_No) %>%
  summarise(
    # Grandparent presence (parent/parent-in-law of head, code 7)
    has_grandparent = any(Relation_to_Head == 7),
    n_grandparents = sum(Relation_to_Head == 7),

    # Gender of grandparents
    has_grandmother = any(Relation_to_Head == 7 & Gender == 2),
    has_grandfather = any(Relation_to_Head == 7 & Gender == 1),
    grandmother_only = any(Relation_to_Head == 7 & Gender == 2) & !any(Relation_to_Head == 7 & Gender == 1),
    grandfather_only = any(Relation_to_Head == 7 & Gender == 1) & !any(Relation_to_Head == 7 & Gender == 2),
    both_grandparents = any(Relation_to_Head == 7 & Gender == 2) & any(Relation_to_Head == 7 & Gender == 1),

    # Grandparent age (average age of grandparents)
    grandparent_avg_age = ifelse(any(Relation_to_Head == 7),
                                  mean(Age[Relation_to_Head == 7], na.rm = TRUE),
                                  NA),

    # Presence of children (grandchildren, married children, unmarried children)
    has_children = any(Relation_to_Head %in% c(3, 5, 6)),
    n_children = sum(Relation_to_Head %in% c(5, 6)), # unmarried children + grandchildren
...
# Load time use data
time_data <- read_dta("TimeUse106PER.dta",
...
set.seed(123)
time_data <- time_data %>% slice_sample(prop = 0.1)
time_data <- time_data %>% mutate(Weight = Weight / 0.1)

# Merge household structure with time use data
data <- time_data %>%
...
# Clean up
rm(data, time_data, demo_data, household_structure)
invisible(gc())
```
