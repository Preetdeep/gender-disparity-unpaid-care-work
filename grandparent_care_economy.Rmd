...
**Results**: Column 1 shows that women in households with grandparents spend less time on childcare (unconditional). Column 2 adds controls for age, marital status, employment, education, and household size; the coefficient magnitude remains substantial, suggesting grandparent households may be somewhat negatively selected on baseline childcare burdens.

**Column 3 is our key specification**: After controlling for eldercare burden, women with co-resident grandparents spend **`r sprintf("%.0f", abs(grandparent_effect_m3))` minutes less per day on childcare** (p < 0.01). This represents a `r sprintf("%.0f%%", pct_reduction)` reduction relative to the mean of `r sprintf("%.0f", mean_childcare)` minutes. Importantly, the eldercare coefficient is `r sprintf("%.2f", eldercare_coef_m3)`, indicating that each minute of eldercare does not fully offset childcare savings.
...
**Results**: The grandparent dividend is **substantially larger for employed mothers** (`r sprintf("%.0f", abs(gp_effect_employed))` minutes) than non-employed mothers (`r sprintf("%.0f", abs(gp_effect_notemployed))` minutes). The interaction term (Column 1) indicates grandparents provide more childcare when mothers work. This is consistent with grandparents responding to household needs and enabling maternal employment.
...
**Results**: Grandmothers provide more childcare support than grandfathers. Women in households with **only a grandmother** spend `r sprintf("%.0f", abs(gm_only_effect))` minutes less on childcare (relative to no grandparent households). With **only a grandfather**, `r sprintf("%.0f", abs(gf_only_effect))` minutes less. Households with **both grandparents** show a `r sprintf("%.0f", abs(both_effect))`-minute reduction, suggesting additive effects rather than diminishing returns.
