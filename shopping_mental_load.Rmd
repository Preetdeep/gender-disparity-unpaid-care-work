...
```{r shopping_plot, fig.width=8, fig.height=5, fig.cap="The Shopping Gap: Women Do 96% of Household Shopping"}
...
employed_female <- employment_table %>% filter(employment_label == "Employed") %>% pull(Female)
not_employed_female <- employment_table %>% filter(employment_label == "Not Employed") %>% pull(Female)
employed_male <- employment_table %>% filter(employment_label == "Employed") %>% pull(Male)
not_employed_male <- employment_table %>% filter(employment_label == "Not Employed") %>% pull(Male)
...
1. **The gap persists among employed women**: Employed women's shopping time (`r sprintf("%.1f", employed_female)` min) is only `r employed_reduction_pct`% lower than unemployed women's (`r sprintf("%.1f", not_employed_female)` min), despite opportunity costs.
...
Employed women face a "double burden" of paid work plus full domestic responsibility (Hochschild 1989). Our results suggest this extends to the mental load: employed women maintain high levels of shopping and planning work despite employment constraints, while employed men do not increase their contributions.
