...
# Model for Remote Socializing (person-day level)
m_remote <- feols(did_remote ~ female + married + female_married + Age + I(Age^2) +
                    higher_ed + employed + urban | state_factor,
                  data = person_day_regression, weights = ~Weight, cluster = ~household_id)

# Model for In-Person Socializing (person-day level)
m_inperson <- feols(did_in_person ~ female + married + female_married + Age + I(Age^2) +
                      higher_ed + employed + urban | state_factor,
                    data = person_day_regression, weights = ~Weight, cluster = ~household_id)
...
```{r coef_plot_digital, fig.cap="Coefficient Plot of Key Predictors on Socializing"}
# Extract coefficients and standard errors for plotting
tidy_remote <- broom::tidy(m_remote) %>% filter(term %in% c("married", "urban", "higher_ed")) %>% mutate(model = "Remote")
tidy_inperson <- broom::tidy(m_inperson) %>% filter(term %in% c("married", "urban", "higher_ed")) %>% mutate(model = "In-Person")
...
# Use person-day regression data (already created above)
# Urban
m_remote_urban <- feols(did_remote ~ female + married + female_married,
                        data = person_day_regression %>% filter(urban==1),
                        weights = ~Weight, cluster = ~household_id)
m_inperson_urban <- feols(did_in_person ~ female + married + female_married,
                          data = person_day_regression %>% filter(urban==1),
                          weights = ~Weight, cluster = ~household_id)

# Rural
m_remote_rural <- feols(did_remote ~ female + married + female_married,
                        data = person_day_regression %>% filter(urban==0),
                        weights = ~Weight, cluster = ~household_id)
m_inperson_rural <- feols(did_in_person ~ female + married + female_married,
                          data = person_day_regression %>% filter(urban==0),
                          weights = ~Weight, cluster = ~household_id)
